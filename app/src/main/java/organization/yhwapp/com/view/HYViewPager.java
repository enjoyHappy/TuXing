...
/**
 * Created by LJZ on 16/10/17. View Pager
 */
public class HYViewPager extends RelativeLayout implements ViewPager.OnPageChangeListener, View.OnClickListener {
	...
	private ArrayList<ImageView> mPagers = new ArrayList<ImageView>();// 滚动的View
	...
	private int color_circle_selected;
	...
	public HYViewPager(Context context, AttributeSet attrs) {
		super(context, attrs);
		if(attrs != null){
			TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.HYViewPager, 0, 0);
			color_circle_normal = a.getColor(R.styleable.HYViewPager_circle_normal,0x112233);
			color_circle_selected = a.getColor(R.styleable.HYViewPager_circle_selected,0x223344);

			MARKER = a.getInteger(R.styleable.HYViewPager_marker_local,CENTER_MARKER);
		}
	...
	private void initViewPager() {
		...
		paramsPoints = new LayoutParams(LinearLayout.LayoutParams.WRAP_CONTENT,
	 ...
	 * 
	 */
	public void setViews(ArrayList<ImageView> views, boolean isNetImage) {
	...
	private void setMarks() {
		...
		for (int i = 0; i < mPoints.length; i++) {
			...
			tvPoints.setText("●");
			tvPoints.setTextColor(color_circle_normal);
			tvPoints.setTextSize(10);
			linPoints.addView(tvPoints);
			if (local == i) {
				tvPoints.setTextColor(color_circle_selected);
			}
		}
		addView(linPoints);
	...
	private class MyPagerAdapter extends PagerAdapter {
		...
		@Override
		public int getCount() {
			return originSize > 1 ? Integer.MAX_VALUE : 1;
		}
	...
	private void setMarkerPosition(int pos) {
		...
		for (int i = 0; i < linPoints.getChildCount(); i++) {
			...
			if (position % mPoints.length == i) {
				tv.setTextColor(color_circle_selected);
			} else {
				tv.setTextColor(color_circle_normal);
			}
		}
	}
